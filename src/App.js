...
function App(props) {
  ...
  useEffect(() => {
    const offset = (state.reducer.page - 1) * 20;
    getArticles(offset);
    ...
    <div className="App">
      <header className={header}>
        ...
        <div className={headerMenu}>
          <HeaderMenu user={state.userReducer.user} onLogOut={onLogOut} logged={state.userReducer.logged} />
        </div>
      </header>
      <main className={main}>
        ...
        <div className={errorsContainer}>
          {state.errorsReducer.errors
            ? Object.keys(state.errorsReducer.errors).map((elem) => (
                <Alert
                  message={elem}
                  closable
                  afterClose={closeErrors}
                  description={state.errorsReducer.errors[elem][0]}
                  type="error"
                  showIcon
                />
              ))
